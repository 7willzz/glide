...
Glide は、Android 向けの高速かつ効率的なオープンソースのメディア管理および画像読み込みフレームワークです。メディアのデコード、メモリおよびディスクキャッシュ、リソースのプーリングを、シンプルで使いやすいインターフェースにまとめています。
...
Glide の主な目的は、あらゆる種類の画像リストのスクロールを可能な限りスムーズかつ高速にすることですが、リモート画像の取得、リサイズ、表示が必要なほぼすべてのケースにおいても効果的です。
...
Glide の使い方は?
-------------------
さまざまなトピックに関するページについては [GitHub wiki][2] を参照し、[javadocs][3] をご覧ください。
...
OkHttp と Volley
-----------------
OkHttp と Volley のサポートは、依存関係として任意で組み込める統合ライブラリによって提供されます。
統合ライブラリは、Maven または [releases ページ][1] から入手できます。
...
ライセンス
-------
BSD、一部 MIT および Apache 2.0。詳細は [LICENSE][16] ファイルを参照してください。

ステータス
------
[*バージョン 3*][14] は、Android Camera アプリや 2014 年の Google IO アプリを含む Google の複数のオープンソースプロジェクトで使用されている安定版の公開リリースです。*バージョン 4* は現在 `master` ブランチで開発中です。
コメント/バグ報告/質問/プルリクエストを歓迎します!
...
**注意**: *Android SDK* に *Android Support Repository* がインストールされていること、および `$ANDROID_HOME` 環境変数が SDK を指していることを確認してください。または、ルートプロジェクトに `sdk.dir=...` の行を含む `local.properties` ファイルを追加してください。

サンプル
-------
[ビルド](#build) セクションの手順に従ってプロジェクトをセットアップし、その後以下を実行します:
...
```
コンパイル済みの APK も [releases ページ][1] で入手できます。

開発
-----------
[ビルド](#build) セクションの手順に従ってプロジェクトをセットアップし、その後自由にファイルを編集してください。
[Intellij IDEA 14][4] は Glide のソースとテストの両方を問題なくインポートでき、Glide の開発に推奨される方法です。
...
ヘルプの取得
------------
特定の問題や機能リクエストを報告するには、[GitHub で新しい issue を開いて][5] ください。質問、提案、その他については、[Glide のディスカッショングループ][6] に参加またはメールを送るか、IRC チャンネル [irc.freenode.net#glide-library][13] に参加してください。
...
謝辞
------
* Glide のディスクキャッシュの基盤となっている[ディスクキャッシュ実装][8] を提供してくれた **Android チーム** と **Jake Wharton**。
* Glide の gif デコーダーの基盤となっている [gif デコーダー gist][9] を提供してくれた **Dave Smith**。
* [gradle-mvn-push][10] スクリプトを提供してくれた **Chris Banes**。
* Glide の[素晴らしいロゴ][11] を作成してくれた **Corey Hall**。
* コードをコントリビュートし、issue を報告してくれたすべての方々!
